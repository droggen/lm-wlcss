...
function  [score,btrackall,dbgscore,dbgsul,dbgsu,dbgsl] = wlcss_int_bt_dbg(motif,stream,penalty,reward,accepteddist,dtype)
...
dbgscore = zeros(size(motif,2),size(stream,2),dtype);
btrackall = zeros(size(motif,2),size(stream,2),'int8');
dbgsul = btrackall;
dbgsu = btrackall;
dbgsl = btrackall;
...
for i=1:size(stream,2)
    [iscore,btracklm,im1,im2,im3] = iwlcss_int_bt_dbg_step(iscore,btracklm,motif,stream(i),penalty,reward,accepteddist,dtype);
