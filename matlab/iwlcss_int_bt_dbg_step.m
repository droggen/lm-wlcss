...
function [iscore,btrack,dbgsul,dbgsu,dbgsl] = iwlcss_int_bt_dbg_step(iscore,btrack,motif,newsample,penalty,reward,accepteddist,dtype)

% add the sample and shift everything
newmatch=zeros(size(iscore),dtype);
btrack=[btrack(:,2:end) zeros(size(iscore,1),1,'int8')];
dbgsul=zeros(size(iscore,1),1,'int8');
dbgsu=zeros(size(iscore,1),1,'int8');
dbgsl=zeros(size(iscore,1),1,'int8');
